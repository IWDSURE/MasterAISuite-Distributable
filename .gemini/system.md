...
- **Never loop on an authentication/connectivity failure.** A `401`/`403`, a failed SQL*Plus connection, or an unreachable KB2 embedding sidecar means stop and escalate per Section 5 — do not retry, and never attempt to regenerate or guess credentials yourself.
...
- **Never push to KB2 outside its 3-step Golden Path** (3.1), leave a new node unlinked, or restructure another node's existing chunks.
...
|---|---|---|---|
| **AI Agents (KB2)** | `kb2.exe` | Graph-based knowledge base — search and record lessons, fixes, specs | `AIAgents_Suite/AIAgents_Suite_README.md` |
...
## 3.1 Start Here: KB2 (AI Agents Suite)

`kb2.exe` is the project's memory. Before researching anything from scratch:
```bash
kb2.exe semantic "<topic in natural language>" --limit 5 --verbose
kb2.exe search "<keyword>" --limit 5
```
Cosine distance < 0.35 = highly relevant.

If you produce a **durable, reusable** finding (a fix, a corrected endpoint, a new mapping), record it via KB2's 3-step Golden Path: **(1) re-query** for related nodes, **(2) plan** — `NEW_LEAF` / `REFINEMENT` / `CORRECTION` — **(3) push** the node + chunks and `link` it (`EVOLVED_FROM` / `SUPERSEDES`) to its ancestor. Don't push transient, task-only findings.

⚠️ If `semantic` fails or returns nothing where results were expected, the **embedding sidecar (`localhost:5002`) is down**. Stop and tell the human to run `START.ps1` in the AI Agents suite's `knowledge-system-graph-based` directory — don't retry, and don't silently fall back to `search`.
...
- **Anything that sounds previously solved, or worth remembering** → KB2 first and last (3.1).
...
1. **Consult KB2** (3.1) for prior art on this exact question.
...
6. **Record durable learnings to KB2** (3.1) if the finding will help future tasks.

**Scope discipline**: distinguish *Inquiries* ("what does this do", "why might this fail") from *Directives* ("find and document the root cause", "extract this report's logic"). For Inquiries, research and answer — don't push to KB2 or create workspace artifacts beyond what your analysis itself needs. For Directives, run the full lifecycle above, pausing only for the triggers in Section 5.
...
| KB2 `semantic` fails or returns nothing unexpectedly | Confirmation the embedding sidecar is running (`START.ps1`) |
...
Fusion tooling can return very large payloads — full table dumps, BIP CSVs, OIC log archives, KB2 verbose chunk dumps. Manage this at the **source**:
...
- Run independent discovery calls in parallel (e.g., `kb2.exe semantic` + `kb2.exe search`, or `db.exe inspect` on several candidate tables) rather than sequentially.
...
For any task spanning 3+ tool calls (most golden paths), use `update_topic` once near the start with your plan (e.g., "Tracing ORA-01722 via KB2 → DB Suite schema check") and once at the end with a recap. Skip it for single lookups.
