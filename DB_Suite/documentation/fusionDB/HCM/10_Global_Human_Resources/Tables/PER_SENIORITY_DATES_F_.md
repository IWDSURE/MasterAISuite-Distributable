...
**Source:** [https://docs.oracle.com/en/cloud/saas/human-resources/oedmh/persenioritydatesf-14369.html#persenioritydatesf-14369](https://docs.oracle.com/en/cloud/saas/human-resources/oedmh/persenioritydatesf-14369.html#persenioritydatesf-14369)

## Primary Key

| Name | Columns |
|------|----------|
| PER_SENIORITY_DATES_F_PK_ | LAST_UPDATE_DATE, LAST_UPDATED_BY, SENIORITY_DATE_ID, EFFECTIVE_START_DATE, EFFECTIVE_END_DATE |
...
| SENIORITY_DATE_CODE | VARCHAR2 | 30 |  |  | Code to identify the Seniority Date Item.Eg: ORA_ESD_P , ORA_JOB_SD_A | Active |
| PERSON_ID | NUMBER |  | 18 |  | Unique identifier (ID) that identifies teh person this item belongs to. | Active |
| ENTRY_DATE | DATE |  |  |  | First date in which the seniority attribute value was first assigned to this person at the given level. | Active |
| SENIORITY_DATE | DATE |  |  |  | The actual seniority date value based on which the length of service will be calculated for day | Active |
| BASE_SENIORITY_DATE | DATE |  |  |  | Seniority Date from the previous date effective row, used as reference for offsetting seniority date in this row. | Active |
| CALCULATION_TYPE | VARCHAR2 | 30 |  |  | Mode of calculation Manual when created by user and  Automatic.when created by ESS | Active |
| LEVEL_CODE | VARCHAR2 | 30 |  |  | Lookup code for the Seniority Level. | Active |
| LEVEL_OBJECT_ID | NUMBER |  | 18 |  | Unique identifier (ID) that identifies the level object. | Active |
| SENIORITY_FIELD | VARCHAR2 | 30 |  |  | Lookup code for the Triggering Field based on which this seniority is defined. | Active |
| SENIORITY_FIELD_KEY | VARCHAR2 | 150 |  |  | Unified redundant column that combines all three seniority key fields - Number, Text, Date |  |
| RECORD_CREATOR | VARCHAR2 | 30 |  |  | Creator process of this record. For example, HDL, UI, and ESS. | Active |
| BUSINESS_GROUP_ID | NUMBER |  | 18 |  | Unique identifier (ID) that identifies the enterprise | Active |
...
|---|---|---|---|---|
| PER_SENIORITY_DATES_F_U1_ | Unique | FUSION_TS_TX_DATA | LAST_UPDATE_DATE, LAST_UPDATED_BY, SENIORITY_DATE_ID, EFFECTIVE_START_DATE, EFFECTIVE_END_DATE | Active |
| PSDFN1_ | Non Unique | FUSION_TS_TX_DATA | SENIORITY_DATE_ID, EFFECTIVE_START_DATE, EFFECTIVE_END_DATE, LAST_UPDATE_DATE | Active |
